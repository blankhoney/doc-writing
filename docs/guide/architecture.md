...
|---|---|---|---|
| 写作规则 | 怎样组织、表达和检查信息？ | `docs/design-spec.md` 及 `docs/modules/` | 要求结论先行、证据可辨、术语一致，并规定检查方法 |
...
**规则保持一个来源，实例保持自己的事实来源。** 扫描器从主规格读取词项，编写和检查指引链接到主规格及模块；示例只展示登记过的写法。修改规则时有明确入口，写项目文档时也能区分通用要求和当前项目的数据。

### 2.2 模型主检与候选扫描

模型主检处理需要上下文的问题，包括主题是否偏移、理由是否支持结论、操作是否具备前提、术语含义是否一致。`runtime/doc-lint.py` 从主规格的 G1 禁用模式清单提取字面词项，输出文件、行号、类别和命中文本，供模型回到原文裁决。
...
| `docs/design-spec.md` | 完整写作约束、适用条件和收尾要求 | 编写前读取完整 §3，验证时重新核对 |
| `docs/modules/` | 模板、表达、读取方式、验证和读者测试的详细规则 | 按阶段及所需功能读取 |
| `templates/_index.md` | 将读者任务对应到文档类型 | 准备阶段 |
| `templates/{type}.md` | 变体选择、锚定节、模块和类型验证 | 类型确定后完整读取 |
| `runtime/write-assist.md` | 组织编写阶段的规则查阅与表达选择 | 开始编写前 |
| `runtime/verify-checks.md` | 组织完整模型主检及后续验证 | 初稿完成后和交付前 |
...
完整通用约束在编写前读取，类型模板按任务选择，表达与验证指引在对应阶段读取。这种按阶段组织资源的方式称为渐进式披露。
...
- [完整写作规则](../design-spec.md)：查阅具体规则的条件、优先级与检查要求。
