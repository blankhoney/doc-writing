...
SCOPE = ("仅检查 G1 字面候选和所选格式；复杂模板不展开，不判断语义例外。"
...
def parse_rules(text):
    ...
    for heading in ("禁用句式", "禁用修饰词", "禁用抽象大词"):
        matches = list(re.finditer(r"^\*\*" + heading + r"\*\*[^\n]*$", block, re.M))
        if len(matches) != 1:
    ...
    rules = []
    rows = [line.strip() for line in block[anchors[0].end():anchors[1].start()].splitlines() if line.strip()]
    cells = [row.strip("|").split("|") for row in rows]
    cells = [[cell.strip() for cell in row] for row in cells]
    if (len(cells) < 3 or cells[0] != ["类别", "模式", "处理"] or len(cells[1]) != 3
    ...
    for row in cells[2:]:
        ...
        if re.search(r"…|\.{2,}|(?<![A-Za-z])[XY](?![A-Za-z])|[<>{}\[\]（）()]", pattern):
            continue
    ...
    modifier = re.fullmatch(r"\*\*禁用修饰词\*\*(?:（[^）]*）)?[：:]\s*(.+)", anchors[1][0].strip())
    if not modifier or block[anchors[1].end():anchors[2].start()].strip():
        raise ValueError("禁用修饰词列表缺失或格式损坏")
    add_words("修饰词", modifier[1])
    bullets = [line.strip() for line in block[anchors[2].end():].splitlines() if line.strip()]
    if not bullets:
...
def mask_markdown(lines):
    ...
    for index, line in enumerate(masked):
        if "`" in line:
            notices.append((index + 1, "范围", "未配对或转义反引号未作代码排除，按正文扫描"))
        # Keep link labels/alt text. Destinations support at most one nested pair.
        line = re.sub(r"(?<=\])\((?:[^()\n]|\([^()\n]*\))*\)", lambda m: blank(m[0]), line)
        if re.search(r"(?<=\])\(", line):
            notices.append((index + 1, "范围", "复杂或未闭合链接目标未完整排除，剩余内容仍扫描"))
        line = re.sub(r"(?<=\])\[[^\]\n]*\]", lambda m: blank(m[0]), line)
        if re.match(r"^(?: {0,3}>[ \t]?)* {0,3}\[[^\]]+\]:[ \t]*\S", line):
            line = blank(line)
        if any(m.end() < len(line) and line[m.end()] == "(" for m in URL.finditer(line)):
            notices.append((index + 1, "范围", "裸 URL 内半角括号未完整排除，括号内容仍扫描"))
    ...
    for index, (raw, visible) in enumerate(zip(lines, masked)):
        ...
        for comment in re.finditer(r"<!--.*?-->", visible):
            ...
            if "doc-lint:" in original:
                directive = re.fullmatch(r"<!--[ \t]*doc-lint: ignore-next-line[ \t]+(.+?)[ \t]*-->", original)
                if directive and directive[1].strip():
                    reasons.append(directive[1].strip())
                else:
                    records.append((number, "范围", "无效抑制注释：仅支持 ignore-next-line 且必须有原因"))
        visible = re.sub(r"<!--.*?-->", lambda m: blank(m[0]), visible)
        if "<!--" in visible:
            records.append((number, "范围", "跨行或未闭合 HTML 注释不解析，按正文扫描"))
        candidates = []
        for category, term in rules:
            candidates.extend((number, category, match[0]) for match in re.finditer(re.escape(term), visible))
        ...
        if pending:
            ...
            records.append((number, "抑制应用", "来自第 %d 行；原因：%s；抑制 %d 个候选"
                            % (pending[0], pending[1], len(candidates))))
        else:
            records.extend(candidates)
        pending = (number, "；".join(reasons)) if reasons else None
    if pending:
        records.append((pending[0], "范围", "抑制未应用：没有下一物理行；原因：" + pending[1]))
    return sorted(records, key=lambda record: record[0]), suppressed


def main(argv=None):
    parser = argparse.ArgumentParser(description="只读扫描文档候选；候选≠违规，0命中≠全文通过。", epilog=SCOPE)
    parser.add_argument("files", metavar="FILE", nargs="+", help="UTF-8 文件（支持 BOM、CRLF 和含空格路径）")
    parser.add_argument("--skip-format", action="append", choices=FORMATS, default=[],
    ...
    for filename in args.files:
        try:
            ...
            records, count = scan_text(path.read_text(encoding="utf-8-sig"), rules, args.skip_format)
    ...
    print(f"统计：完成 {completed}/{len(args.files)} 个文件；显示候选 {candidates}；抑制 {suppressed}；"
