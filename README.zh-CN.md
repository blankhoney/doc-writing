...
面向支持 Agent Skills 标准的模型与框架，用于技术文档和审核文档的编写、补全与检查。给出材料和目标，助手按文档类型组织内容、核对事实、编写正文，并完成表达与结构检查。

适合编写项目需求、开发前架构方案、技术设计、API 文档、部署手册、使用指南和架构说明，以及测试报告、决策记录等审核材料，也可以检查或更新已有文档。当前规则、模板和详细指南主要使用中文。

下文以 Claude Code 演示安装和调用。包内入口目前使用该客户端的路径变量和工具约定；其他框架需适配入口、技能发现和工具映射。本包在其他框架中的直接运行能力及适配后的兼容性尚未逐一验证。

## 快速开始

在 Claude Code 中安装后，在目标项目会话中输入：
...
此请求以已有 `README.md` 为材料，并授权写入指定路径。只想先审阅正文时，改为“只在对话中输出”。编写文档不授权执行正文描述的部署、付款等操作，也不授权提交或发布结果；这些动作需要单独授权。
...
- **从读者任务确定结构**：选择文档类型、变体和必要模块，保留必需章节，按任务决定深度。技术设计区分架构方案与程序详细设计；详细设计按需交代接口、失败处理、依赖与包布局。
- **在执行前读取适用规则**：先读共同约束，再按准备、取材、编写和验证阶段读取对应原文。所选模板完整读取，表格、代码块、图和示例指导按需加载；首次输出正文或骨架前读编写规则，提出架构决策前读架构约束。
...
扫描器读取文件并返回候选位置，正文修改由使用者或助手决定。用词与格式规则面向中文；扫描英文 README 仅用于运行检查，不提供英文文风审查。
...
项目原创代码、规则和文档采用 [MIT License](LICENSE)。Requests、Backstage、Django 和 Kubernetes enhancements 的示例片段分别保留原项目的许可证和署名；PEP 380 片段保留作者 Gregory Ewing 的公有领域声明。详见[来源记录](examples/SOURCES.md)及 [examples/licenses/](examples/licenses/)。
